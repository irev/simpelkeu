...
class Simda extends CI_Controller
{
    ...
    {
		$data['title'] = "SIMDA";
        //$this->load->view('simda/simda_list');
        $this->template->load('template','simda/simda_list',$data);
    } 
	...
	public function cari(){
		...
		if($q!=''){
			$data = $this->Simda_model->search_data($q);
		}
		if($id!=''){
			$data = $this->Simda_model->search_data_byId($id);
		}
		//echo $this->db->last_query();
		return $this->output->set_content_type('application/json')->set_output(json_encode(['total_count'=> $data->num_rows(),'dinas'=>$data->row('COL22'),'items'=>$data->result()]));
	}
///
    public function read($id) 
    {
		...
		'COL40' => $row->COL40,
            ...
            $this->load->view('simda/simda_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('simda'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'title' => 'simda',
            'button' => 'Create',
            'action' => site_url('simda/create_action'),
	    'id' => set_value('id'),
	    'COL1' => set_value('COL1'),
	    'COL2' => set_value('COL2'),
	    'COL3' => set_value('COL3'),
        ...
        //$this->load->view('simda/simda_form', $data);
        $this->_render_page('simda/simda_form', $data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'title' => 'simda',
                'button' => 'Update',
                'action' => site_url('simda/update_action'),
...
/* End of file Simda.php */
/* Location: ./application/controllers/Simda.php */
